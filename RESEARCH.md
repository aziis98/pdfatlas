...
### 1.18. Capping Page-Texture Render Zoom at 300% (`texture_zoom`)
...
- **Solution:** Cap the **zoom setting** used for page-texture rendering at `MAX_TEXTURE_ZOOM = 3.0` (300%):
  $$\text{render\_zoom} = \text{layout\_scale}(\min(\text{zoom}, 3.0), \text{dpi\_scale\_factor}), \qquad \text{texture\_res} = \text{render\_zoom} \times \text{scale\_factor}$$
  via `texture_zoom()` in [`pdfatlas/core/layout.py`](pdfatlas/core/layout.py). Applied consistently at the two scale consumers (`PDFCanvas.render_zoom()` feeds both `_request_render` job queuing/gating and `GLCanvas._on_render` cache lookups).
  Beyond 300% zoom the quad is drawn at the full logical `dw × dh` size while the texture stays at the 3.0× resolution — `GL_LINEAR` upscales it ("rescaling technique"). Overlay math (`scale = zoom × dpi_scale_factor`) is deliberately **not** capped so selection/highlight/link rects stay pixel-correct at true zoom.
- **Configurable in Settings:** the cap is a `CropSettings.max_texture_zoom` field (default `3.0`), editable from the Settings dialog as a text field whose placeholder is **"Infinity"** — clearing the field sets it to `None` (no cap, textures render at true zoom). `min_zoom` / `max_zoom` settings (defaults `0.25` / `50.0`) replace the hardcoded clamp in `NavigationController.set_zoom_level`; the window re-clamps the current zoom when the limits change.
- **Why logical-zoom cap (not physical-pixel cap):** capping physical pixels per PDF point (the earlier rejected `max_physical_zoom` clamp at 2.66×) made HiDPI (2×) displays start rescaling at 150% zoom, blurring text through the 150–300% range. Capping the zoom *setting* keeps textures 1:1 sharp up to 300% zoom on every display while still bounding memory at the 300%-zoom-equivalent size.
- **Cache consistency:** the stored cache scale (`render_zoom × scale_factor`), the gating check, and the draw lookup all use the capped value, so zooming past 300% reuses the same cached texture instead of re-rendering; `get_best` proximity picks it during pinch too.
