...
- **Multiprocessing child backend exclusively retained:** The rasterization pipeline previously allowed switching between multiprocessing (`mp`) and multithreaded (`mt`) backends. The in-process `mt` backend was retired: PyMuPDF `fitz.Document` objects are not thread-safe to share, and GIL re-acquisition bursts in background threads caused main-thread UI hitches. The multiprocessing backend (`RenderWorker` + `render_child.py`) with zero-copy shared memory IPC is now the sole rendering backend.
...
- **Headless Profiling Harness:** Combined `py-spy record --format flamegraph --subprocesses` with `scripts/screenshot_wayland_app.sh` (using `WAIT_FOR_EXIT=1`) to capture multi-process flame graphs headlessly without manual user interaction.
...
### 1.35. Single-Instance D-Bus Application & Multi-Tab Document Model
- **Finding:** In Libadwaita / GTK4 applications, managing multiple documents with tabs across windows is most robustly handled using a single-instance `Gio.ApplicationFlags.HANDLES_COMMAND_LINE` structure:
  1. **D-Bus CLI Argument Dispatching:** `PDFViewerApplication.do_command_line` handles arguments passed from external invocations (`pdfatlas paper.pdf` or arXiv URL clicks), resolving paths or arXiv identifiers and opening in a new window by default (`MainWindow(self, ...)` with `open_document(source, new_tab=False)`), or populating an existing empty window if one is present.
