...
- **Finding:** In Libadwaita / GTK4 applications, managing multiple documents with tabs across windows is most robustly handled using a single-instance `Gio.ApplicationFlags.HANDLES_COMMAND_LINE` structure:
  1. **D-Bus CLI Argument Dispatching:** `PDFViewerApplication.do_command_line` handles arguments passed from external invocations (`pdfatlas paper.pdf` or arXiv URL clicks), resolving paths or arXiv identifiers and opening new tabs in the existing active window (`win.open_document(source, new_tab=True)`).
