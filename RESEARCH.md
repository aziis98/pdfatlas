...
- **Solution:** Render every highlight rect into an offscreen RGBA framebuffer (`CompositingLayer` in [`pdfatlas/ui/gl_renderer.py`](pdfatlas/ui/gl_renderer.py)) using Screen blending *between* highlights, then multiply-composite the layer over the page frame:
  1. The layer is cleared to **transparent black** (`0.0, 0.0, 0.0, 0.0`); each rounded rect is drawn into the layer with `GL_ONE, GL_ONE_MINUS_SRC_COLOR` screen blending *between* highlights, additively combining overlapping rects ($C_1 + C_2 - C_1 \cdot C_2$).
  2. `fill_round_rect` uses a shader branch (`u_is_placeholder == 3` in `quad.frag`) that computes rounded-corner coverage from a pixel-space SDF (`cov = clamp(0.5 - (length(max(abs(p) - b, 0)) - r), 0, 1)`), outputting premultiplied color `vec4(u_color.rgb * alpha, alpha)`.
  3. After the page loop, `composite_layer` converts un-highlighted pixels to white (`u_is_placeholder == 4` in `quad.frag`: `mix(vec3(1.0), tex.rgb, tex.a)`) and draws the layer with `GL_DST_COLOR, GL_ZERO` multiply blending on top of the page frame — multiplying screen-blended highlight colors over page content while leaving un-highlighted pixels untouched ($1.0 \times C_{\text{page}} = C_{\text{page}}$).
