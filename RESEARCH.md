...
- **Geometry:** rects are expanded **symmetrically +2 PDF pt** on every side and rounded with a **2 pt radius** (`pad = radius = 2.0 * scale`). The layer texture is allocated at physical pixel resolution (`logical × gl_scale`) but sampled with normalized coords, keeping corners crisp on HiDPI.
- **Layout coupling:** the composite quad must undo the scroll offset — `QuadRenderer.begin` records `_offset_x/_offset_y` and `composite_layer` draws with `u_page_pos = offset` so the layer quad covers the exact viewport.
